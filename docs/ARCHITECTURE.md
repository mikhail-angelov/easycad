...
| `refiner.py` | Stage-1 **triage**. One LLM call classifies a request vs the current model → `ready` / `refine` / `clarify` / `invalid`; returns a refined prompt, questions, or a reason in the selected interface language. |
