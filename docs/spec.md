...
> **⚠️ Superseded — retro-analysis only.** This is the earlier "drawing → Feature
> Graph → trusted CadQuery compiler" architecture (image/sketch input). The
> shipped product was rebuilt from scratch as a text-prompt CadQuery chat; the
> current architecture is defined by `spec11.md` onward (`spec11.md`–`spec14.md`
> plus the SPEC15 skill-discovery work). None of the machinery below (Feature
> Graph, backend compiler, `implemented/approximated/unsupported` states,
> experimental markers) exists in the running app. Kept for historical reference
> only — do not treat its statements (e.g. "complete modeled threads" being
> unsupported) as describing the current system. Real helical threads are now
> generated by the thread skill (SPEC15).
